...
const getQuickLinks = computed(() =>
  [
    archive.value,
    ...hasSub.value
      .filter((item) => ["sootechestvenniki", "archive"].includes(item.slug))
...
<template>
  <div
    ...
    :class="[
      'hidden lg:block absolute z-40 w-full transform transition-all duration-300 ease-out overflow-hidden',
      `h-[calc(100dvh-62px)] xl:h-[calc(100dvh-var(--header-height))] `,
      open ? 'translate-y-0 top-full' : '-translate-y-full top-0',
    ]"
  >
    <div
      class="h-full bg-gradient-to-b from-white via-white/95 to-white dark:from-gray-800/80 dark:via-gray-800/90 dark:to-gray-800 overflow-hidden"
    >
      <div
        class="flex flex-col justify-between h-full py-5 xl:py-10 overflow-hidden"
      >
        <div class="container h-full flex-1 overflow-auto">
          <div class="columns-2 gap-4 md:columns-3 lg:columns-4">
            ...
            <!-- <div
              class="bg-gray-50 dark:bg-gray-700 rounded-xl p-2 xl:p-4 h-fit grid grid-cols-2 gap-5"
            >
              <div
            ...
            <div
              class="bg-gray-200 dark:bg-gray-700 rounded-xl p-2 xl:p-4 h-fit"
            >
              <div
        ...
        <div
          class="ml-5 px-5 pt-4 xl:pt-5 border-t-2 border-gray-300 dark:border-gray-500/20"
        >
          <div class="container">
