...
// faqat bitta index ochiq bo‘ladi
const activeIndex = ref<number | null>(null)
const contentRefs = ref<HTMLElement[]>([])

const toggle = (index: number): void => {
	if (activeIndex.value === index) {
		// agar shu ochiq bo‘lsa → yopiladi
		activeIndex.value = null
	} else {
		// boshqasini yopib yangisini ochamiz
...
<template>
	<div class="space-y-4">
		...
		>
			...
			<div
				ref="contentRefs"
				:class="['overflow-hidden transition-all duration-300 ease-in-out pl-2 pr-4', activeIndex === index ? 'mt-6' : '']"
				:style="{
